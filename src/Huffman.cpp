...
void archiveFile(string sourceFilename, string resultFilename){
    ...
    alphabet = getAlphabet(sourceFile, sourceFileLength);
 ...
 * --------------------------------------------------------------------------------------------
 *
 * This function read all characters in the recieved file stream and put them and their
 * frequencies to the Map. Also it change the value of variable sourceFileLength, what will
...
Map<char, int> parseCodeString(string codeString){
    ...
    for (int i = 0; i < codeString.length(); i++){
        ...
        if(valueTrig && codeString[i] != ';'){ //read value until met with the separator ";"
            valueString += codeString[i];
            continue;
        }

        if(!keyTrig && codeString[i] == ';'){
            result.put(key, stringToInteger(valueString)); // write combination of key and value
